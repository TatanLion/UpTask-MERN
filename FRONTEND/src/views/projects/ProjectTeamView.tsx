...
export default function ProjectTeamView() {
  ...
  const { mutate } = useMutation({
    mutationFn: ({ projectId, id }: removeUserFromProjectProps) => removeUserFromProject(projectId, id),
    onSuccess: (data) => {
  ...
  const handleRemoveUser = (id: User['_id']) => {
    mutate({ projectId, id });
  }
  ...
  if (data) return (
    <>
      ...
      <p className="text-2xl font-light text-gray-500 mt-5">Aquí puedes gestionar los miembros de tu equipo</p>
